...
import { generateBoard, getSolvedBoard, stringifyBoard } from './auxiliaryFunctions';
...
export const boardReducer = function (state = initialState, action) {
    switch (action.type) {
        case GET_BOARD: {
            let newLevel;
            !action.diffLevel ? newLevel = state.diffLevel : newLevel = action.diffLevel;
            const initial = generate(newLevel),
                solved = getSolvedBoard(initial),
                current = generateBoard(initial);
            return { ...state, initialBoard: initial, currentBoard: current, solvedBoard: solved, diffLevel: newLevel, isWon: false};
        }
        case SOLVE_BOARD: {
            const solvedStringifiedBoard = getSolvedBoard(state.currentBoard, state.solvedBoard),
